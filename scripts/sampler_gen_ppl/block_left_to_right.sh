#!/bin/bash
#SBATCH -J gen_ppl_bd3lm_block_left_to_right               # Job name
...
for k in "${sampling_params[@]}"; do
    echo "Generating samples with block_left_to_right k=$k"
    srun python -u main.py \
        ...
        wandb.project=duel +wandb.name=sampler-genppl-block-l2r \
        mode=sample_eval \
        ...
        +sampling.strategy=block_left_to_right \
        +sampling.strategy_k=$k \
        sampling.logdir=$PWD/sample_logs/samples_bd3lm_len${LENGTH}_blocksize${BLOCK_SIZE}_block-left-to-right_k${k}.txt
done