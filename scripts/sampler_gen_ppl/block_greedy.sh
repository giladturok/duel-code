#!/bin/bash
#SBATCH -J genppl_bd3lm_block_greedy  # Job name
...
for k in "${sampling_params[@]}"; do
    echo "Generating samples with block_greedy k=$k"
    srun python -u main.py \
        ...
        wandb.project=duel +wandb.name=sampler-genppl-block-greedy \
        mode=sample_eval \
        ...
        +sampling.strategy=block_greedy \
        +sampling.strategy_k=$k \
        sampling.logdir=$PWD/sample_logs/samples_bd3lm_len${LENGTH}_blocksize${BLOCK_SIZE}_block-greedy_k${k}.txt 
done