...
def process_docs(dataset: datasets.Dataset) -> datasets.Dataset:
    """
    Process Winogrande dataset to conform to common multiple choice format.
    Creates full sentence completions for each option.
    """
    def _process_doc(doc):
        ...
        # Create complete sentences by replacing "_" with each option
        prefix = sentence[:blank_idx]
        suffix = sentence[blank_idx + 1:]  # Skip the "_" character
        
        choice1 = prefix + doc["option1"] + suffix
        choice2 = prefix + doc["option2"] + suffix
        
        # Convert answer "1" or "2" to 0-indexed label
        answer_to_label = {"1": 0, "2": 1}
        label = answer_to_label[doc["answer"]]
        
        out_doc = {
            "sentence": sentence,  # Keep for reference
